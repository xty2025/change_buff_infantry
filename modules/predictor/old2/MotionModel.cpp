...
Prediction MotionModel::getPredictResult(const Time::TimeStamp& timestamp, int carid)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if((std::get<2>(armor_measure_temp) - timestamp).toSeconds() < 1 && std::get<0>(armor_measure_temp) == i)
        {
            result.armors[i].x = std::get<1>(armor_measure_temp)[2] * cos(std::get<1>(armor_measure_temp)[0]) * cos(std::get<1>(armor_measure_temp)[1]);
            result.armors[i].y = std::get<1>(armor_measure_temp)[2] * cos(std::get<1>(armor_measure_temp)[0]) * sin(std::get<1>(armor_measure_temp)[1]);
            result.armors[i].z = std::get<1>(armor_measure_temp)[2] * sin(std::get<1>(armor_measure_temp)[0]);
...
void MotionModel::Update(const VectorY& measure, const Time::TimeStamp& timestamp, int armor_id)
{
    if(firstUpdate)
    {
        ...
        armor_measure_temp = std::make_tuple(armor_id, measure, timestamp);
